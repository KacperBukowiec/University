...
def storms(rows, cols, triples):
    ...
    cs = domains(variables) + startowe(triples) + rzedy(rows, cols) + kolumny(rows, cols) + kwadraty(len(rows), len(
        cols)) + sasiednie(len(rows), len(cols))
...
#print(list(map(int,txt[0].split(" "))))
