...
;Korzystałem z wykładów i wikipedii
;Niestedy nie miałem siły robić testów przez chorobe, proszę o wyrozumiałość
...
(define (solve-ax+by=1 a b)
  (define (iter s t r old_s old_t old_r)
    (if (= r 0)
        (cons old_s old_t)
        (iter old_s old_t old_r (- old_s (* (quotient old_r r) s)) (- old_t (* (quotient old_r r) t)) (- old_r (* (quotient old_r r) r)))))
  (iter 0 1 b 1 0 a))
...
(define (RSA-unconvert-list intlist key2)
  (let ((n (key-modulus key2)))
    (define (unconvert l sum)
      (if (null? l)
          '()
          (let ((x (modulo (+ sum (RSA-transform (car l) key2)) n)))
...
;;; XXX: Define the data structure that represents signed messages he
...
(define (encrypt-and-sign message recipient-public-key sender-private-key)
  (make-signed-message (RSA-transform (compress message) sender-private-key) (RSA-encrypt message recipient-public-key))) 
  
  ;;; XXX: Write your code here
  ;;;(error "Not implemented"))

(define (authenticate-and-decrypt cyphertext recipient-private-key sender-public-key) ;;;przyjmuje że cyphertext jest parą (podpis wiadomość)
  (if (= (compress (RSA-decrypt (cdr cyphertext) recipient-private-key)) (RSA-decrypt (car cyphertext) sender-public-key))
      (RSA-decrypt (cdr cyphertext) recipient-private-key)
